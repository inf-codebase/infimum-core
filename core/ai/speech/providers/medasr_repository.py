...
# librosa imported locally in functions that need it
import decouple

from .transcription_entity import TranscriptionEntity, WordConfidence
...
class MedASRRepository:
    ...
    def __init__(
        ...
        self.token = (
            token or os.getenv("HF_TOKEN") or os.getenv("HUGGING_FACE_HUB_TOKEN")
        )
        if not self.token:
            try:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if (
                        "Unrecognized processing class" in error_str
                        or "Can't instantiate" in error_str
                    ):
                        try:
                ...
                try:
                    ...
                    if (
                        "lasr_ctc" not in error_str.lower()
                ...
                error_str = str(e)
                if (
                    "lasr_ctc" in error_str.lower()
                    or "does not recognize this architecture" in error_str.lower()
                ):
                    raise RuntimeError(
            ...
            error_msg = str(e)
            if (
                "gated" in error_msg.lower()
                or "401" in error_msg
                or "restricted" in error_msg.lower()
            ):
                raise RuntimeError(
    ...
    def transcribe_from_bytes(
        ...
        return self.transcribe(
            audio_array, sample_rate, return_confidences, return_timestamps
        )
