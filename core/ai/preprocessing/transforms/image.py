...
import numpy as np
from ...core.preprocessing.base import BaseTransform
...
class ResizeTransform(BaseTransform):
    ...
    def transform(self, data: DataItem) -> DataItem:
        ...
        if data.data_type != "image":
            raise ValueError(f"ResizeTransform expects image data, got {data.data_type}")
...
class NormalizeTransform(BaseTransform):
    ...
    def transform(self, data: DataItem) -> DataItem:
        ...
        """
        if data.data_type != "image":
            raise ValueError(f"NormalizeTransform expects image data, got {data.data_type}")
...
class AugmentTransform(BaseTransform):
    ...
    def transform(self, data: DataItem) -> DataItem:
        ...
        """
        if data.data_type != "image":
            raise ValueError(f"AugmentTransform expects image data, got {data.data_type}")
