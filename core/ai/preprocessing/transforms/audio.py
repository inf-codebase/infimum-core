...
class ResampleTransform(BaseTransform):
    ...
    def transform(self, data: DataItem) -> DataItem:
        ...
        if data.data_type != "audio":
            raise ValueError(f"ResampleTransform expects audio data, got {data.data_type}")
...
class NormalizeAudioTransform(BaseTransform):
    ...
    def transform(self, data: DataItem) -> DataItem:
        ...
        """
        if data.data_type != "audio":
            raise ValueError(f"NormalizeAudioTransform expects audio data, got {data.data_type}")
