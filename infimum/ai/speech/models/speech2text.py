...
from ..providers.transcription_entity import TranscriptionEntity
...
class Speech2Text:
    ...
    def transcribe(
        self, audio_source: Union[str, Path, bytes], language: Optional[str] = None
    ) -> str:
        """
        ...
        if isinstance(audio_source, Path):
            ...
            raise ValueError(
                "Bytes input not yet supported. Please provide a file path."
            )
        else:
