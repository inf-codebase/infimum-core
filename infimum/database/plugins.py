...
def register_builtin_backends() -> None:
    ...
    """
    from infimum.utils import auto_config
    all_configs = auto_config._config_items()

    try:
        if "milvus".upper() in (k.upper() for k, v in all_configs):
    ...
    try:
        if "qdrant".upper() in (k.upper() for k, v in all_configs):
    ...
    try:
        if "mongo".upper() in (k.upper() for k, v in all_configs):
            from infimum.database.mongo import MongoManager
    ...
    try:
        if "postgres".upper() in (k.upper() for k, v in all_configs):
